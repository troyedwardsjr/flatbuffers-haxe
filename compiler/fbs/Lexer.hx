...
class Lexer extends hxparse.Lexer implements hxparse.RuleBuilder {
	...
	static public var keywords = @:mapping(3) FbsKeyword;

	static inline function mk(def:FbsTokenDef, lexer:hxparse.Lexer) return new FbsToken(def, lexer.curPos());
	...
	public static var tok = @:rule [
		...
		"\\." => mk(TDot, lexer),
		"\\.\\.\\." => mk(TEllipsis, lexer),
		//"(true|false)" => mk(TBool((cast lexer.current:String).toLowerCase() == "true" ? true : false), lexer),
		"(true|false)" => mk(TBool(lexer.current), lexer),
